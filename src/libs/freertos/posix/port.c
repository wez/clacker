...
static volatile uintptr_t uxCriticalNesting;
...
portBASE_TYPE xPortStartScheduler(void) {
  ...
  /* Start the timer that generates the tick ISR.  Interrupts are disabled here
   * already. */
...
void vPortYieldFromISR(void) {
  /* Calling Yield from a Interrupt/Signal handler often doesn't work because
   * the
   * xSingleThreadMutex is already owned by an original call to Yield.
   * Therefore,
...
void prvSetupSignalsAndSchedulerPolicy(void) {
  /* The following code would allow for configuring the scheduling of this task
  as a Real-time task.
   * The process would then need to be run with higher privileges for it to take
  affect.
  int iPolicy;
  int iResult;
  int iSchedulerPriority;
          iResult = pthread_getschedparam( pthread_self(), &iPolicy,
  &iSchedulerPriority );
          iResult = pthread_attr_setschedpolicy( &xThreadAttributes, SCHED_FIFO
  );
          iPolicy = SCHED_FIFO;
          iResult = pthread_setschedparam( pthread_self(), iPolicy,
  &iSchedulerPriority );		*/
...
void vPortFindTicksPerSecond(void) {
  ...
  printf(
      "Timer Resolution for Run TimeStats is %ld ticks per second.\n",
      ulTicksPerSecond);
}
