# This is tri by Martijn Meijers
# from https://bitbucket.org/bmmeijers/tri/src
# It was made available under the MIT license
...
def output_triangles(T, fh):
    ...
    for t in T:
        ...
        fh.write("{0};{1};{2[0]};{2[1]};{2[2]};{3[0]};{3[1]};{3[2]};{4};{5}\n".format(id(t), t, [
                 id(n) for n in t.neighbours], [id(v) for v in t.vertices], t.is_finite, t.info))
...
class Triangle(object):
    ...
    def __init__(self, a, b, c):
        ...
        # FIXME: could be coded as a bitmask on an integer
...
def triangulate(points, infos=None, segments=None):
    ...
    if infos is not None or segments is not None:
        index_translation = dict([(pos, newpos)
                                  for (newpos, (_, _, pos)) in enumerate(points)])
        if segments is not None:
            # -- translate the segments
            segments = [(index_translation[segment[0]],
                         index_translation[segment[1]]) for segment in segments]
...
class ConstraintInserter(object):
    ...
    def insert(self, segments):
        ...
        for j, segment in enumerate(segments):
            p = self.triangulation.vertices[segment[0]]
            q = self.triangulation.vertices[segment[1]]
            try:
    ...
    def remove_empty_triangles(self):
        ...
        """
        new = list(filter(lambda x: not(x.vertices[0] is None or x.vertices[1]
                                        is None or x.vertices[2] is None), self.triangulation.triangles))
        logging.debug(str(len(self.triangulation.triangles)) +
                      " (before) versus " + str(len(new)) + " (after) triangle clean up")
...
def test_poly():
    ...
    if False:
        ...
        with open("/tmp/centers.wkt", "w") as fh:
            fh.write("wkt\n")
            for incenter in trafo.centers.values():
...
def test_small():
    ...
    triangulate([(0, 0), (10, 6), (6.5, 1.5), (2, 3), (40, -20),
                 (15, -4), (-120, 90), (-60, 40), (-45, 35)])
