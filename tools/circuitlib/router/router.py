...
def routew(data):
    ...
    # First, lets order the two nets such that fixed layer requirements are
    # routed first when we attempt a solution below.  We use the layers
    # attribute to figure this out; if the node has a single layer then
    # we must consider it first, otherwise we assume that it is present on
    # all layers and has more flexibility in routing.
    two_nets = sorted(two_nets, key=lambda x: len(
        x.layers) if hasattr(x, 'layers') else 3)

    def avoid_obstacle(a, b, obstacle, layer=None):
        avoid = obstacle.buffer(0.5, cap_style=CAP_STYLE.square,
                                join_style=JOIN_STYLE.mitre).simplify(types.TRACK_RADIUS / 4)
    ...
    def compute_weights():
        ...
        for a, b in tqdm(g.edges_iter(), desc='recompute weights'):
            cost = a.shape.centroid.distance(b.shape.centroid)
    ...
    # Iteratively improve the paths to route around physical features on the
    # board.  This doesn't care about intersecting with other paths.
    mutated = True
    ...
    tqdm.write('graph has %d nodes and %d edges' % (len(g), g.size()))

    # The next step is to consider intersection of the paths.  The game plan
    # is to compute edges to represent traversing layers; each of those edges
    # will belong to a layer and we can use that to weight the path accordingly.
    layer_g = networkx.Graph()
    layer_map = {types.FRONT: {}, types.BACK: {}}
    for node in tqdm(g, desc='build layer graph nodes'):
        vertical = []
        for layer in node.layers:
            if isinstance(node, (types.ThruHole, types.SmdPad)):
                # Create a proxy node on the appropriate layer
                proxy = types.Branch(node.shape, layer)
                layer_g.add_edge(node, proxy, weight=0, layer=layer)
                layer_map[layer][node] = proxy
                vertical.append(proxy)
            elif isinstance(node, types.Branch):
                # Make a copy of this node for this layer
                proxy = types.Branch(node.shape, layer)
                layer_map[layer][node] = proxy
                layer_g.add_node(proxy)
                vertical.append(proxy)
            else:
                raise Exception("unhandled node type")
        if len(vertical) == 2:
            layer_g.add_edge(vertical[0], vertical[1], via=True)

    for a, b in tqdm(g.edges_iter(), desc='build layer graph edges'):
        for layer in set(a.layers + b.layers):
            a_proxy = layer_map[layer].get(a)
            b_proxy = layer_map[layer].get(b)
            if a_proxy and b_proxy:
                layer_g.add_edge(a_proxy, b_proxy, layer=layer)
            else:
                tqdm.write('missing %s %s or %s %s' % (a, a_proxy, b, b_proxy))

    class Solution(object):
        def __init__(self, g):
            self.g = g.copy()
            self.smap = spatialmap.SpatialMap()
            self.cost = 0
            self.paths = []

        def compute_weights(self):
            ''' Recompute the weights for each edge '''
            for a, b in tqdm(self.g.edges_iter(), desc='recompute layer weights'):
                cost = a.shape.centroid.distance(b.shape.centroid)
                ...
                if layer:
                    track = track_shape(a, b)
                    for entry in list(smap.intersects(track)) + list(self.smap.intersects(track)):
                        if hasattr(entry.value, 'layers'):
                            if layer not in entry.value.layers:
                                continue
                        if entry.shape == a.shape or entry.shape == b.shape:
                            continue
                        cost += COLLISION_COST
                self.g[a][b]['weight'] = cost

        def route_2net(self, a, b):
            # We need to find a path from a -> b with an acceptable cost
            cost, path = networkx.bidirectional_dijkstra(
                self.g, a, b, weight='weight')
            self.paths.append(path)
            for i, j in pairwise(path):
                if i.shape.centroid != j.shape.centroid:
                    track = track_shape(i, j)
                    self.smap.add(track, (i, j))
            self.compute_weights()
            return cost, path

        def solve(self, two_nets):
            self.compute_weights()
            for a, b in tqdm(two_nets, desc='solve route'):
                cost, path = self.route_2net(a, b)
                self.cost += cost

    solution = Solution(layer_g)
    solution.solve(two_nets)

    routed_graph = networkx.Graph()
    for path in tqdm(solution.paths, desc='distil route'):
        for i, j in pairwise(path):
            routed_graph.add_node(i)
            routed_graph.add_node(j)
            routed_graph.add_edge(i, j, weight=solution.g[i][j]['weight'])
